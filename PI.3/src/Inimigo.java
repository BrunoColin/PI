...
public class Inimigo extends Inimigos {
	...
	public Inimigo(BufferedImage image, float vel, float x, float y, int charx, int chary) {
		...
		if (state == ATACANDO) {
			...
			if (atacando == false) {
				...
				for (int i = 0; i < CanvasGame2.salas.get(CanvasGame2.controlaSalas).tiles.size(); i++) {
					...
					if (rectColision(CanvasGame2.salas.get(CanvasGame2.controlaSalas).tiles.get(i))) {
