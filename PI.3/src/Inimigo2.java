...
public class Inimigo2 extends Inimigos {
	...
	public Inimigo2(BufferedImage image, float vel, float x, float y, int charx, int chary) {
		...
		visao = new CampoVisao((int) x, (int) y, 15, null);
	...
	public void CalculaIA(long diftime) {
		dxD = point2ds.get(pos).getX() - x;
		dyD = point2ds.get(pos).getY() - y;
		...
		if (state == ATACANDO) {
			...
			if (atacando == false) {
				// vel = 0;
				contAtacando += diftime;
				ang = Math.atan2(CanvasGame2.player.y - y, CanvasGame2.player.x - x);
				angVisao = ang;
				if (contAtacando > cdAtacando) {
				...
				// for(int i = 0; i <
				// CanvasGame2.salas.get(CanvasGame2.controlaSalas).tiles.size();
				// i++){
				//
				// if(rectColision(
				// CanvasGame2.salas.get(CanvasGame2.controlaSalas).tiles.get(i))){
				//
				// contAtacando2 = 0;
				// atacando = false;
				// state = ALERTA;
		...
		if (state == ALERTA) {
			...
			if (vigiando == false) {
				...
				if (setCaminho) {
					...
					caminho = setaObjetivo((int) lastPosPlayer.getX() / 16, (int) lastPosPlayer.getY() / 16);
					setCaminho = false;
				}
		...
		if (state == VOLTANDO) {
			...
			if (caminho != null) {
				...
				if ((estado * 2) < caminho.length) {
					...
					// System.out.println(point2ds.get(pontoObjetivo).getX());
	...
	public int[] setaObjetivo(int objetivox, int objetivoy) {
		...
		// System.out.println(((int)(x+charx/2) / 16));
		int caminho[];
	...
	public void DesenhaSe(Graphics2D dbg, int xMundo, int yMundo) {
		...
		if (dead == false) {
			...
			if (atacando) {
				...
				dbg.drawRect((int) x + xMundo, (int) y + yMundo, charx + xMundo, chary + yMundo);
